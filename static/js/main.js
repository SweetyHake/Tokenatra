...
function updateDeviceIndicator(device, modelExists) {
    ...
    if (aboutValue) aboutValue.textContent = !modelExists ? '???????????? ???? ??????????????????????' : deviceName;
...
function initWindowControls() {
    ...
    // ???????????????????????????? ????????: Windows ??? ?????????? Win32 (flask('move')),
    // Linux/macOS ??? ?????????? pywebview-drag-region ???? ??????????????????.
    if (IS_WINDOWS) {
...
function initEdgeResize() {
    ...
    // Windows-only: ???? Linux/macOS ???????????? frameless-???????? ???????????? pywebview.
...
function initModelSelector() {
    ...
    fetch('/models_list').then(function(r) { return r.json(); }).then(function(d) {
        ...
        d.models.forEach(function(m) {
            const opt = document.createElement('option');
            opt.value = m.name;
            opt.textContent = m.selected ? m.name + ' (' + I18n.t('??????????????') + ')' : m.name;
    ...
    async function runModelAction(button, endpoint, busyText) {
        ...
        if (statusText) statusText.textContent = I18n.t(endpoint.includes('download') ? '???????????????? ???????????? ?? ?????????????????' : '?????????????????????? ???????????????');
        try {
    ...
    if (serverBtn && !serverBtn.dataset.bound) {
        ...
        serverBtn.onclick = () => runModelAction(serverBtn, '/download_model_from_server', '???????????????????');
    }
    if (diskBtn && !diskBtn.dataset.bound) {
        diskBtn.dataset.bound = '1';
        diskBtn.onclick = () => runModelAction(diskBtn, '/load_model_from_disk', '?????????? ?????????????');
    }
}
    ...
    function poll() {
        fetch('/update_status').then(function(r) { return r.json(); }).then(function(d) {
            if (d && d.update_available && !d.download_done && d.update_tag) {
                ...
                if (badge) {
                    badge.hidden = false;
                    badge.dataset.tooltip = '???????????????? ???????????????????? v' + d.update_tag;
                }
                toast('???????????????? ???????????????????? v' + d.update_tag);
                return;
            }
...
function pollAboutUpdate(retries) {
    fetch('/update_status').then(r => r.json()).then(d => {
        ...
        if (d.download_error) {
            // ???????????? ???? ???????????????????? updater ??? ???????????????????? ?????????? ???????????????? ?? HTML
            aboutUpdateSetStatus('<span style="color:#ef4444">???????????? ????????????????????: ' + escapeHtml(d.download_error) + '</span>');
            if (actions) actions.innerHTML = '<button class="about-link" onclick="hideAboutUpdate()">??????????????</button>';
            aboutUpdating = false;
            aboutDownloading = false;
        ...
        if (d.download_done) {
            ...
            aboutUpdateSetStatus('???????????????????? <b>' + escapeHtml(d.update_tag || '') + '</b> ??????????????. ???????????????????? ?????????????');
            if (actions) actions.innerHTML =
                '<button class="accent-btn accent-btn-compact" onclick="applyAboutUpdate()">????????????????????</button>' +
                '<button class="about-link" onclick="hideAboutUpdate()">??????????</button>';
            aboutUpdating = false;
            aboutDownloading = false;
            if ($('checkUpdatesBtn')) $('checkUpdatesBtn').disabled = false;
            return;
        ...
        if (d.update_checked && d.check_error) {
            aboutUpdateSetStatus('<span style="color:#ef4444">???? ?????????????? ?????????????????? ???????????????????? (?????? ???????????????????? ?? GitHub)</span>');
            if (actions) actions.innerHTML = '<button class="about-link" onclick="hideAboutUpdate()">??????????????</button>';
            aboutUpdating = false;
            aboutDownloading = false;
            if ($('checkUpdatesBtn')) $('checkUpdatesBtn').disabled = false;
        ...
        if (d.update_available && d.update_url) {
            aboutUpdateSetStatus('???????????????? ???????????? <b>' + escapeHtml(d.update_tag || '') + '</b> (??????????????: ' + escapeHtml(d.current_version || '') + ')');
            if (actions) actions.innerHTML =
                '<button class="accent-btn accent-btn-compact" onclick="startAboutDownload()">??????????????</button>' +
                '<button class="about-link" onclick="hideAboutUpdate()">??????????????</button>';
            aboutUpdating = false;
            if ($('checkUpdatesBtn')) $('checkUpdatesBtn').disabled = false;
            // ???????? ???????????????? ?????? ????????????????????, ???? ???????? ?????? ???? ???????????? ??? ???????????????????? ????????????????????
...
function applyAboutUpdate() {
    ...
    fetch('/apply_update', { method: 'POST' })
        ...
        .then(d => {
            if (!d || !d.ok) {
                aboutUpdateSetStatus((d && d.error) ? d.error : '???? ?????????????? ???????????????????? ????????????????????');
                return;
            }
            aboutUpdateSetStatus('?????????????????? ??????????????????????? ???????????????????? ??????????????????');
            fetch('/api/window/destroy', { method: 'POST' }).catch(() => {});
        })
        .catch(() => aboutUpdateSetStatus('???? ?????????????? ?????????????????? ?? ????????????????'));
}
